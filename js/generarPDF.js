...
async function pintarModeloPDF(){
        ...
        const responseCabecera = await fetch(`https://apiconexcot.somee.com/api/cotizaciones/${idCotizacion}`);
        if (!responseCabecera.ok) throw new Error('Error al obtener cabecera de cotización');
        const cabecera = await responseCabecera.json();
 
        const responseDetalle = await fetch(`https://apiconexcot.somee.com/api/cotizacionesdet/${idCotizacion}`);
